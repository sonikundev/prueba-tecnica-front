...
interface FormData {
  title: string;
  description: string;
  status: { name: string; value: string } | null;
  price: number | null;
}
...
const validateForm = () => {
  ...
  if (!formData.title.trim()) {
    ...
    errors.title = "El nombre no puede exceder 25 caracteres.";
    valid = false;
  } else {
    errors.title = "";
  }
  ...
  if (!formData.description.trim()) {
    ...
    errors.description = "La descripción no puede exceder 70 caracteres.";
    valid = false;
  } else {
    errors.description = "";
  }
...
const onSave = async () => {
  ...
  try {
    ...
    toast.add({
      ...
      detail: `Se agregó correctamente "${formData.title}"`,
      life: 8000,
    });
